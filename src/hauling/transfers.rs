    ...
    T: HasStore + MaybeHasId + JsCast,
{
    ...
    if transfer_stage != TransferStage::BeforeAnyTransfers {
        if let Some(resource_type) = resource_type {
            let transfers = current_tick_transfers(object_id.into(), resource_type);
            if transfer_stage == TransferStage::AfterOutgoingTransfers || transfer_stage == TransferStage::AfterAllTransfers {
                amount -= transfers.outgoing as i32;
            }
            if transfer_stage == TransferStage::AfterIncomingTransfers || transfer_stage == TransferStage::AfterAllTransfers {
